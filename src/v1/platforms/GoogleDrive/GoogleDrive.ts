...
export class GoogleDrive implements StoragePlatform {
    ...
    public async getTokens(
        ...
        try {
            ...
            ({ clientId, clientSecret } = await InstanceManager.get(
                instanceKey,
                [MapKey.clientId, MapKey.clientSecret]
            ));
            const oAuth2Client: OAuth2Client = new google.auth.OAuth2(
                clientId,
                clientSecret,
                `https://${hostName}/auth/callback/google`
            );
    ...
    async endUpload(
        ...
        const { name, webViewLink, id, fileExtension, webContentLink } =
            googleFileCreationResult.data;
