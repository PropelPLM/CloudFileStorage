...
const init = () => {
    io.on('connection', (socket: any) => {
        socket.on('start', async (instanceKey: string) => {
            ...
            try {
                let salesforceUrl;
                ({ salesforceUrl } = await InstanceManager.get(instanceKey, [
                ...
                logErrorResponse(
                    err,
                    '[MESSAGE_EMITTER.JOIN_ROOM.SET_ATTRIBUTE]'
                );
            }
        });
    });
...
const setAttribute = (
    ...
    try {
        ...
        logSuccessResponse(
            { instanceKey, ...keyedAttribute },
            '[MESSAGE_EMITTER.SET_ATTRIBUTE]'
        );
    } catch (err) {
